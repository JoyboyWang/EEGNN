...
class BasicDataLoader(object):
    ...
    def _load_data(self):
        ...
        self.erase_mat = np.empty(self.num_data, dtype=object)
...
class SingleDataLoader(BasicDataLoader):
    ...
    def _prepare_data(self):
        ...
        for sample in tqdm(self.data):
            ...
            next_id += 1
        print("The edge in dataset is " + str(edge))
        num_no_query_ent = 0
        num_one_query_ent = 0
        num_multiple_ent = 0
        for i in range(next_id):
...
class SingleDataLoaderDeleteEdge(BasicDataLoader):
    """
    Single Dataloader creates training/eval batches during KGQA.
    """
    def __init__(self, config, word2id, relation2id, entity2id, tokenize, data_type="train"):
        super(SingleDataLoaderDeleteEdge, self).__init__(config, word2id, relation2id, entity2id, tokenize, data_type)
    ...
    def _build_fact_mat(self, sample_ids, fact_dropout):
        ...
        batch_tails = np.array([], dtype=int)
        batch_ids = np.array([], dtype=int)
        erase_heads = np.array([], dtype=int)
        erase_rels = np.array([], dtype=int)
        erase_tails = np.array([], dtype=int)
        erase_ids = np.array([], dtype=int)
        erase_dict = {}
        ...
        for i, sample_id in enumerate(sample_ids):
            ...
            all_indices = set(range(num_fact))
            mask_index_set = set(mask_index)
            inverse_mask_index = list(all_indices - mask_index_set)
            ...
            real_erase_head_list = head_list[inverse_mask_index] + index_bias
            real_erase_tail_list = tail_list[inverse_mask_index] + index_bias
            real_erase_rel_list = rel_list[inverse_mask_index]

            batch_heads = np.append(batch_heads, real_head_list)
            batch_rels = np.append(batch_rels, real_rel_list)
            batch_tails = np.append(batch_tails, real_tail_list)
            batch_ids = np.append(batch_ids, np.full(len(mask_index), i, dtype=int))
            erase_heads = np.append(erase_heads, real_erase_head_list)
            erase_rels = np.append(erase_rels, real_erase_rel_list)
            erase_tails = np.append(erase_tails, real_erase_tail_list)
            erase_ids = np.append(erase_ids, np.full(len(inverse_mask_index), i, dtype=int))
            if self.use_self_loop:
                num_ent_now = len(self.global2local_entity_maps[sample_id])
                ent_array = np.array(range(num_ent_now), dtype=int) + index_bias
                rel_array = np.array([self.num_kb_relation - 1] * num_ent_now, dtype=int)
        ...
        for head, tail, rel, bid in zip(erase_heads, erase_tails, erase_rels, erase_ids):
            if str(bid) not in erase_dict:
                erase_dict[str(bid)] = {}
	        
            if str(head - 2000 * bid) not in erase_dict[str(bid)]:
                erase_dict[str(bid)][str(head - 2000 * bid)] = {}
	        
            erase_dict[str(bid)][str(head - 2000 * bid)][str(tail - 2000 * bid)] = int(rel)
        #ipdb.set_trace()
        with open('edge.json', 'w') as file2:
            json.dump(erase_dict, file2, indent=4)
    ...
    def _prepare_data(self):
        ...
        num_query_entity = {}
        edge = 0
        #with open('10%erase.json', 'r') as file:
            #erase_list = json.load(file)
        #erase_i = 0
        #erase_dict = {}
        ...
        for sample in tqdm(self.data):
            ...
            for i, tpl in enumerate(sample['subgraph']['tuples']):
                ...
                #100 %
                #if False:
               #if erase_i < len(erase_list) and erase_list[erase_i]:
               #    erase_head_list.append(head)
               #    erase_tail_list.append(tail)
               #    erase_rel_list.append(rel)
	       ...
	       #
            ...
            if not self.data_eff:
                self.kb_adj_mats[next_id] = (np.array(head_list, dtype=int),
                                         np.array(rel_list, dtype=int),
                                         np.array(tail_list, dtype=int))
                self.erase_mat[next_id] = (np.array(erase_head_list, dtype=int),
                                         np.array(erase_rel_list, dtype=int),
                                         np.array(erase_tail_list, dtype=int))
		

            next_id += 1
        #EE Edit Generate Edge json
        #ipdb.set_trace()
        print("The edge in dataset is " + str(edge))
        num_no_query_ent = 0
        num_one_query_ent = 0
...
def load_data(config, tokenize):
    ...
    if config["is_eval"]:
        ...
        valid_data = SingleDataLoader(config, word2id, relation2id, entity2id, tokenize, data_type="dev")
        test_data = SingleDataLoaderDeleteEdge(config, word2id, relation2id, entity2id, tokenize, data_type="test")
